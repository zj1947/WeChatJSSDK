...
import com.z.wechatjssdk.webview.EventDistributor;
    ...
    public JsCallJava (String injectedName, Class injectedCls) {
        ...
        try {
            ...
            sb.append(" call error, message:miss method name\"}" +
                    ...
                    "if(g.code!=200){throw\"");
            sb.append(mInjectedName);
            sb.append(" call error, code:\"+g.code+\", message:\"+g.result}return g.result};Object.getOwnPropertyNames(a).forEach(function(d){var c=a[d];" +
                    "if(typeof c===\"function\"&&d!==\"callback\"){a[d]=function(){return c.apply(a,[d].concat(Array.prototype.slice.call(arguments,0)))}}});b.");
            sb.append(mInjectedName);
            sb.append("=a;console.log(\"");
            sb.append(mInjectedName);
            sb.append(" initialization end\")})(window);");
            mPreloadInterfaceJS = sb.toString();
            Log.d(TAG, "mPreloadInterfaceJS:" + mPreloadInterfaceJS);
        } catch(Exception e){
    ...
    public String call(WebView webView, String jsonStr) {
        if (!TextUtils.isEmpty(jsonStr)) {
            try {
                ...
                //currMethod.invoke(null, values)执行HostJsScope里面相应的函数
    ...
    public String functionReturnCall(WebView webView,String strJSON){
        if (!TextUtils.isEmpty(strJSON)) {
            try {
                ...
                int jsCallBackIndex=0;

                if (len==3){

                        currType = argsTypes.optString(0);
                        if ("string".equals(currType)) {
                ...
                //优先返回详细的错误信息
                if (e.getCause() != null) {
                    return getReturn(strJSON, 500, "method execute error:" + e.getCause().getMessage());
                }
                return getReturn(strJSON, 500, "method execute error:" + e.getMessage());
            }
        } else {
            return getReturn(strJSON, 500, "call data empty");
        }
    }
